...
OpenRouter traffic goes through internal Next.js route handlers under `src/app/api/openrouter`. If `OPENROUTER_API_KEY` is configured on the server, the app can use that hosted key when the browser does not provide its own; a user-supplied key still takes precedence and is routed through the same proxy to OpenRouter, but is not persisted across reloads. Shared replays are immutable snapshots stored separately from live debate execution.
...
- Starter bundles and personal API keys are not persisted across page reloads; personal keys still transit the app proxy to reach OpenRouter.
